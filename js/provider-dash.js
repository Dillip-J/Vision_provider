...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadScheduleManager() {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            dateCon.innerHTML += `
                <button class="btn-day-select" data-day="${dayFull}" style="text-align: center; padding: 12px 16px;">
                    <strong style="font-size: 1.4rem; display: block; margin-bottom: 4px;">${dateNum}</strong>
                    <span style="font-size: 0.85rem; text-transform: uppercase;">${dayShort}</span>
                </button>
            `;
        }
        ...
        let activeDay = new Date().toLocaleDateString('en-US', { weekday: 'long' });
        ...
        if(dayBtns.length > 0) { 
            ...
            renderTimes(dayBtns[0].getAttribute('data-day')); 
        }
    }
    ...
    async function loadProfileSettings() {
        ...
        if(emailEl) emailEl.value = currentProvider.email || ''; 
